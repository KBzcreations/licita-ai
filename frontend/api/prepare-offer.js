...
export default async function handler(req, res) {
  ...
  const { action, licitacion, perfil = {}, documentAnalysis = null, sessionId, tenderId = '', title = '' } = req.body || {};
  ...
  if (action === 'checkout') {
    ...
    params.append('line_items[0][price_data][product_data][name]', 'Expediente de preparación Licita AI');
    ...
    params.append('integration_identifier', 'licita_dossier_aqkzpmrx');
    try { const r = await fetch('https://api.stripe.com/v1/checkout/sessions', { method: 'POST', headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/x-www-form-urlencoded', 'Stripe-Version': '2026-07-29.dahlia' }, body: params.toString() }), s = await r.json(); return r.ok && s.url ? res.status(200).json({ url: s.url }) : res.status(r.status || 502).json({ error: 'Stripe no pudo iniciar el pago' }); }
  ...
  return res.status(200).json(deterministicOfferPack(licitacion, perfil, documentAnalysis));
}
