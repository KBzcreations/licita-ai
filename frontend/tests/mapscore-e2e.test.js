...
test('flujo DEOGA: búsqueda Google, análisis con Groq parcial y correo Resend confirmado',async()=>{
  ...
  global.fetch=async(url,options={})=>{
    const u=String(url);
    if(u.endsWith('places:searchText'))return response({places:[{id:'deoga-id',displayName:{text:'Deoga Desarrollo Y Ejecucion de Obra SL'},formattedAddress:'El Charcón, 110, Loc, Genil, 18008 Granada',primaryTypeDisplayName:{text:'Contratista general'},rating:4.4,userRatingCount:10,websiteUri:'http://www.deoga.es/',nationalPhoneNumber:'958 22 52 51',businessStatus:'OPERATIONAL',photos:Array.from({length:10},(_,i)=>({name:`photo-${i}`}))}]});
    if(u.includes('/v1/places/deoga-id'))return response({rating:4.4,userRatingCount:10,websiteUri:'http://www.deoga.es/',nationalPhoneNumber:'958 22 52 51',businessStatus:'OPERATIONAL',photos:Array.from({length:10},(_,i)=>({name:`photo-${i}`})),reviews:[{rating:5,text:{text:'Buen trabajo'}}]});
    if(u==='http://www.deoga.es/')return new Response('<title>DEOGA Granada</title><meta name="description" content="Construcción"><h1>DEOGA</h1>',{status:200});
    if(u.includes('api.groq.com'))return response({choices:[{message:{content:'{"resumen_ejecutivo":"","hallazgos":[]}'}}]});
    if(u==='https://api.resend.com/emails'&&options.method==='POST'){sent=JSON.parse(options.body);return response({id:'email_123'})}
    if(u==='https://api.resend.com/emails/email_123')return response({id:'email_123',last_event:'delivered',to:['cliente@example.com']});
  ...
  const report=r.body;r=res();await deliver({method:'POST',body:{email:'cliente@example.com',business:place.name,result:report,access_token:token()}},r);assert.equal(r.statusCode,200);assert.equal(r.body.ok,true);assert.equal(r.body.accepted,true);assert.equal(r.body.id,'email_123');assert.equal(r.body.state,'delivered');assert.equal(r.body.delivered,true);assert.equal(sent.to[0],'cliente@example.com');assert.match(sent.html,/DEOGA/);assert.match(sent.html,/KPIs/);
});
...
test('Resend sin id no se marca como enviado',async()=>{process.env.STRIPE_SECRET_KEY='sk_test';process.env.RESEND_API_KEY='resend';global.fetch=async()=>response({},200);const {deterministicReport}=await import('../api/_mapscore-report.js');let r=res();await deliver({method:'POST',body:{email:'cliente@example.com',business:'DEOGA',result:deterministicReport({name:'DEOGA',address:'Granada',hours:[]},{}),access_token:token()}},r);assert.equal(r.statusCode,502);assert.equal(r.body.error,'Resend no aceptó el envío');});
